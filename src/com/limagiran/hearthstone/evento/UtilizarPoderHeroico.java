...
public class UtilizarPoderHeroico {
    ...
    private static boolean paladin_default(int value) {
        if (partida.getHero().temEspacoNaMesa()) {
            ...
            GameCliente.showPopUp("Não há espaço na mesa!");
            return false;
        }
    }
    ...
    private static boolean shaman_default() {
        if (partida.getHero().temEspacoNaMesa()) {
            Card criado = partida.criarCard(Utils.getTotemXamaPoderHeroico(partida.getHero().getMesa()), System.nanoTime());
            if (criado != null) {
                partida.getHero().evocar(criado);
                return true;
            } else {
                GameCliente.showPopUp("Todos os Totens foram evocados!");
                return false;
            }
        } else {
            GameCliente.showPopUp("Não há espaço na mesa!");
