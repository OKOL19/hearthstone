...
public class UltimoSuspiro {
    ...
    public static void processar(Card card, int index) {
        ...
        if (partida.isVezHeroi()) {
            ...
            for (String evocar : card.getEvocarUltimoSuspiro()) {
            ...
            for (String add : card.getAddMaoUltimoSuspiro()) {
                card.getDono().addMao(partida.criarCard(add, System.nanoTime()));
            }
            for (String add : card.getAddDeckUltimoSuspiro()) {
                card.getDono().addCardDeckAleatoriamente(partida.criarCard(add, System.nanoTime()));
            }
        }
    }
    ...
    private static void at_050() {
        card.getDono().setPoderHeroico(SHAMAN_AT_050T);
    }
    ...
    private static void gvg_096() {
        card.getDono().evocar(partida.criarCard(Random.getCard(Values.LACAIO, Values.GERAL, 2, Values.GERAL,
                        Values.GERAL, Values.GERAL).getId(), System.nanoTime()));
    }
    ...
    private static void gvg_105() {
        card.getDono().evocar(partida.criarCard(Random.getCard(Values.LACAIO, Values.GERAL, 4, Values.GERAL,
                        Values.GERAL, Values.GERAL).getId(), System.nanoTime()));
    }
