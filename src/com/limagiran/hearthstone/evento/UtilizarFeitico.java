...
public class UtilizarFeitico {
    ...
    /**
     * Executar o método referente ao feitiço utilizado
     *
     * @param id id do feitiço
     * @throws JogouCardException feitiço cancelado
     */
     ...
     * @throws com.limagiran.hearthstone.heroi.control.JogouCardException
     */
    public static void switchFeitico(Partida partida, String id, long alvo)
    ...
    private static boolean anularAlterarFeitico() throws JogouCardException {
        GameCliente.setBlockSend(true);
        List<Card> list = new ArrayList<>();
        oponente.getSegredo().stream().forEach((segredo) -> {
            list.add(segredo);
        });
    ...
    private static boolean validarArma(Heroi h) throws JogouCardException {
        if (h.getArma() != null) {
            ...
            alerta("Você não tem arma equipada!");
            throw new JogouCardException("Feitiço cancelado.");
        }
    }
    ...
    private static boolean validarQuantidade(Heroi hero, int quantidade) throws JogouCardException {
        if (hero.getMesa().size() >= quantidade) {
            ...
            alerta("Quantidade Insuficiente!");
            throw new JogouCardException("Feitiço cancelado.");
        }
        return true;
    }
    ...
    /**
     * Exibe uma mensagem de alerta na mesa de jogo
     *
     * @param mensagem
     */
    public static void alerta(String mensagem) {
        GameCliente.showPopUp(mensagem);
    }
    ...
    private static void at_016() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            todosOsLacaios.stream().forEach((lacaio) -> {
    ...
    private static void at_024() throws JogouCardException {
        if (validarAlvoByRace(DEMONIO) && !anularAlterarFeitico()) {
            if (alvoCard != null) {
    ...
    private static void at_033() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 2; i++) {
                Card random = Random.getCard(FEITICO, oponente.getType(),
                        Random.QUALQUER_CUSTO, GERAL, GERAL, GERAL);
    ...
    private static void at_035() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 3; i++) {
                oponente.addCardDeckAleatoriamente(partida.criarCard(EMBOSCADA, System.nanoTime()));
            }
        }
    }
    ...
    private static void at_037b() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(BROTO1_1, System.nanoTime()));
            hero.evocar(partida.criarCard(BROTO1_1, System.nanoTime()));
        }
    }
    ...
    private static void at_043() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            if (hero.getMana() < 10) {
                ...
                mao.stream().forEach((c) -> {
                    hero.descartar(c);
                });
            } else {
                hero.addMao(partida.criarCard(EXCESSO_DE_MANA, System.nanoTime()));
            }
        }
    }
    ...
    private static void at_062() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 3; i++) {
                hero.evocar(partida.criarCard(TECETEIA1_1, System.nanoTime()));
            }
        }
    }
    ...
    private static void at_068() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.getMesa().stream().filter((c) -> (c.isProvocar()))
                    .forEach((c) -> {
                        c.addVidaMaxima(2);
                        c.addAtaque(2);
                    });
    ...
    private static void at_074() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico()) {
            if (alvoCard != null) {
                alvoCard.addAtaque(3);
                alvoCard.addMechanics(ESCUDO_DIVINO);
            }
        }
    }
    ...
    private static void brm_005() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            partida.getMesa().stream().filter((lacaio) -> (!lacaio.isDemonio()))
                    .forEach((lacaio) -> {
                        lacaios.add(lacaio);
                    });
    ...
    private static void cs1_112() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            todos.stream().filter((lacaio) -> (lacaio.getVida() <= 0))
                    .forEach((lacaio) -> {
                        GameCliente.send(Param.ANIMACAO_CARD_MORREU,
                                lacaio.id_long);
    ...
    private static void cs2_009() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico()) {
            if (alvoCard != null) {
                alvoCard.addMechanics(PROVOCAR);
                alvoCard.addVidaMaxima(2);
    ...
    private static void cs2_011() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            partida.getEfeitoProgramado().concederAtaqueAoHeroiUmTurno(2);
            hero.getMesa().stream().forEach((lacaio) -> {
                partida.getEfeitoProgramado()
                        .concederAtaqueAUmLacaioUmTurno(lacaio.id_long, 2);
            });
        }
    }
    ...
    private static void cs2_022() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico()) {
            if (alvoCard != null) {
                partida.polimorfia(alvoCard.id_long, OVELHA1_1, true);
            }
        }
    }
    ...
    private static void cs2_026() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            oponente.getMesa().stream().forEach((lacaio) -> {
    ...
    private static void cs2_027() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(IMAGEM_ESPELHADA0_2, System.nanoTime()));
            hero.evocar(partida.criarCard(IMAGEM_ESPELHADA0_2, System.nanoTime()));
        }
    }
    ...
    private static void cs2_039() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico()) {
            if (alvoCard != null) {
                alvoCard.addMechanics(FURIA_DOS_VENTOS);
            }
        }
    }
    ...
    private static void cs2_046() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.getMesa().stream().forEach((lacaio) -> {
                partida.getEfeitoProgramado()
                        .concederAtaqueAUmLacaioUmTurno(lacaio.id_long, 3);
            });
        }
    }
    ...
    private static void cs2_103() throws JogouCardException {
        if (validarAlvoLacaiosHeroi() && !anularAlterarFeitico() && alvoCard != null) {
            alvoCard.addAtaque(2);
            alvoCard.addMechanics(INVESTIDA);
        }
    }
    ...
    private static void dream_02() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            partida.getMesa().stream()
                    .filter((lacaio) -> (!lacaio.getId().equals(YSERA)))
                    .forEach((lacaio) -> {
                        lacaios.add(lacaio);
                    });
            oponente.delHealth(dano);
    ...
    private static void ex1_128() throws JogouCardException {
        if (validarQuantidade(hero, 1) && !anularAlterarFeitico()) {
            hero.getMesa().stream().forEach((lacaio) -> {
                partida.getEfeitoProgramado()
                        .concederFurtividadeAUmLacaioAteProximoTurno(lacaio.id_long);
            });
        }
    }
    ...
    private static void ex1_158() throws JogouCardException {
        if (validarQuantidade(hero, 1) && !anularAlterarFeitico()) {
            hero.getMesa().stream().forEach((lacaio) -> {
                lacaio.addEvocarUltimoSuspiro(ARVOROSO2_2);
            });
        }
    }
    ...
    private static void ex1_160a() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(PANTERA3_2, System.nanoTime()));
        }
    }
    ...
    private static void ex1_244() throws JogouCardException {
        int quantidade = hero.getMesa().stream()
                .filter((lacaio) -> (lacaio.isTotem()))
                .map((_item) -> 1).reduce(0, Integer::sum);
        if (quantidade > 0) {
            if (!anularAlterarFeitico()) {
                hero.getMesa().stream().filter((lacaio) -> (lacaio.isTotem()))
                        .forEach((lacaio) -> {
                            lacaio.addVidaMaxima(2);
                        });
            }
        } else {
            alerta("Você não tem Totem na mesa!");
            throw new JogouCardException("Feitiço cancelado.");
        }
    }
    ...
    private static void ex1_246() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico() && alvoCard != null) {
            partida.polimorfia(alvoCard.id_long, SAPO0_1PROVOCAR, true);
        }
    }
    ...
    private static void ex1_248() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(LOBO_ESPIRITUAL2_3PROVOCAR, System.nanoTime()));
            hero.evocar(partida.criarCard(LOBO_ESPIRITUAL2_3PROVOCAR, System.nanoTime()));
        }
    }
    ...
    private static void ex1_317() {
        List<Card> demonios = Filtro.raca(hero.getDeck(), DEMONIO);
        if (demonios.isEmpty()) {
            hero.addMao(partida.criarCard(DIABRETE_INUTIL, System.nanoTime()));
            hero.addMao(partida.criarCard(DIABRETE_INUTIL, System.nanoTime()));
    ...
    private static void ex1_320() throws JogouCardException {
        if (danoPersonagem(2)) {
            if (alvoCard != null && alvoCard.getDono().getMorto().contains(alvoCard)) {
                hero.evocar(partida.criarCard(Random.byRace(DEMONIO).getId(), System.nanoTime()));
            }
        }
    }
    ...
    private static void ex1_407() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            partida.getMesa().stream()
                    .filter((lacaio) -> (!lacaio.equals(aleatorio)))
                    .forEach((lacaio) -> {
                        destruir.add(lacaio);
                    });
    ...
    private static void ex1_409() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            if (hero.getArma() != null) {
                ...
                hero.setWeapon(partida.criarCard(ARMA1_3, System.nanoTime()));
            }
        }
    }
    ...
    private static void ex1_410() throws JogouCardException {
        if (hero.getShield() > 0) {
            ...
            alerta("Você não tem pontos de Armadura!");
            throw new JogouCardException("Feitiço cancelado.");
        }
    }
    ...
    private static void ex1_538() throws JogouCardException {
        if (validarQuantidade(oponente, 1) && !anularAlterarFeitico()) {
            ...
            for (int i = 0; i < quantidade; i++) {
                hero.evocar(partida.criarCard(CAO1_1, System.nanoTime()));
            }
        }
    }
    ...
    private static void ex1_544() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            partida.getMesa().stream()
                    .filter((lacaio) -> (lacaio.isFurtivo()))
                    .forEach((lacaio) -> {
                        lacaio.delMechanics(FURTIVIDADE);
                    });
    ...
    private static void ex1_549() throws JogouCardException {
        if (validarAlvoAliadoByRace(FERA)
                && !anularAlterarFeitico() && alvoCard != null) {
            partida.getEfeitoProgramado()
                    .concederAtaqueAUmLacaioUmTurno(alvoCard.id_long, 2);
            partida.getEfeitoProgramado()
                    .concederImunidadeAUmLacaioUmTurno(alvoCard.id_long);
        }
    }
    ...
    private static void ex1_571() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 3; i++) {
                hero.evocar(partida.criarCard(
                        ARVOROSO2_2INVESTIDAMORREFIMTURNO,
                        System.nanoTime()));
            }
        }
    }
    ...
    private static void ex1_578() throws JogouCardException {
        if (hero.getAttack() > 1) {
            ...
            alerta("Personagem sem valor de ataque!");
            throw new JogouCardException("Feitiço cancelado.");
        }
    }
    ...
    private static void ex1_619() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            partida.getMesa().stream().forEach((lacaio) -> {
    ...
    private static void ex1_625() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            if (!hero.getPoderHeroico().getTipo().equals(PoderHeroico.PRIEST_EX1_625B)
                    && hero.getPoderHeroico().getTipo().equals(PoderHeroico.PRIEST_EX1_625)) {
                hero.setPoderHeroico(PoderHeroico.PRIEST_EX1_625B);
            } else {
                hero.setPoderHeroico(PoderHeroico.PRIEST_EX1_625);
            }
        }
    }
    ...
    private static void fp1_019() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            for (int i = 0; i < quantidadeHeroi; i++) {
                hero.evocar(partida.criarCard(ARVOROSO2_2T2,
                        System.nanoTime()));
            }
            for (int i = 0; i < quantidadeOponente; i++) {
                oponente.evocar(partida.criarCard(ARVOROSO2_2T2,
                        System.nanoTime()));
            }
        }
    }
    ...
    private static void fp1_025() throws JogouCardException {
        if (validarAlvoLacaios() && !anularAlterarFeitico() && alvoCard != null) {
            alvoCard.morreu();
            alvoCard.getDono().evocar(partida.criarCard(alvoCard.getId(),
                    System.nanoTime()));
        }
    }
    ...
    private static void gvg_001() throws JogouCardException {
        if (validarQuantidade(oponente, 1) && !anularAlterarFeitico()) {
            Utils.getLacaioAleatorio(oponente.getMesa())
                    .delVida(Utils.getDanoFeitico(4, hero));
        }
    }
    ...
    private static void gvg_003() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            Card aleatorio = partida.criarCard(Random.lacaio().getId(),
                    System.nanoTime());
    ...
    private static void gvg_005() throws JogouCardException {
        if (validarQuantidade(hero, 1) && !anularAlterarFeitico()) {
            hero.getMesa().stream().forEach((lacaio) -> {
                hero.addMao(partida.criarCard(lacaio.getId(),
                        System.nanoTime()));
            });
        }
    }
    ...
    private static void gvg_008() throws JogouCardException {
        if (!partida.getMesa().isEmpty()) {
            if (!anularAlterarFeitico()) {
                ...
                lacaios.stream().filter((lacaio) -> (lacaio.getVida() <= 0))
                        .forEach((lacaio) -> {
                            lacaio.morreu();
                        });
            }
        } else {
            alerta("Não há lacaios na mesa!");
            throw new JogouCardException("Feitiço cancelado.");
        }
    }
    ...
    private static void gvg_012() throws JogouCardException {
        if (curarPersonagem(3)) {
            if ((alvoCard != null && !alvoCard.isIleso())
                    || (alvoHeroi != null && !alvoHeroi.isIleso())) {
                hero.evocar(partida.criarCard(GUARDIA_DA_LUZ,
                        System.nanoTime()));
            }
        }
    }
    ...
    private static void gvg_026() throws JogouCardException {
        if (validarQuantidade(hero, 1) && !anularAlterarFeitico()) {
            hero.getMesa().stream().forEach((lacaio) -> {
                UltimoSuspiro.processar(lacaio, lacaio.getDono()
                        .getPosicaoNaMesa(lacaio.id_long));
            });
        }
    }
    ...
    private static void gvg_031() throws JogouCardException {
        if (validarAlvoLacaiosOponente() && !anularAlterarFeitico()
            ...
            partida.addHistorico(alvoCard.getName()
                    + " foi reembaralhado no deck do dono.");
    ...
    private static void part_002() throws JogouCardException {
        if (validarAlvoLacaiosHeroi() && !anularAlterarFeitico()
                && alvoCard != null) {
            partida.voltarPraMaoDoDono(alvoCard.id_long);
        }
    }
    ...
    private static void part_004() throws JogouCardException {
        if (validarAlvoLacaiosHeroi() && !anularAlterarFeitico()
                && alvoCard != null) {
            partida.getEfeitoProgramado()
                    .concederFurtividadeAUmLacaioAteProximoTurno(alvoCard.id_long);
        }
    }
    ...
    private static void gvg_041b() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 5; i++) {
                hero.evocar(partida.criarCard(FOGO_FATUO, System.nanoTime()));
            }
        }
    }
    ...
    private static void gvg_045() throws JogouCardException {
        ...
        if (validarAlvoLacaios() && !anularAlterarFeitico() && alvoCard != null) {
            ...
            for (int i = 0; i < evocar; i++) {
                hero.evocar(partida.criarCard(DIABRETE1_1T2, System.nanoTime()));
            }
        }
    }
    ...
    private static void gvg_050() throws JogouCardException {
        if (partida.getFeiticosAtivos().isNEW1_036()
                && oponente.getMesa().isEmpty()) {
            throw new JogouCardException("Feitiço cancelado.");
    ...
    private static void gvg_061() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            for (int i = 0; i < 3; i++) {
                hero.evocar(partida.criarCard(RECRUTA_PUNHO_DE_PRATA,
                        System.nanoTime()));
            }
            hero.setWeapon(partida.criarCard(ARMA1_4, System.nanoTime()));
        }
    }
    ...
    private static void loe_002() throws JogouCardException {
        if (danoPersonagem(3)) {
            hero.addCardDeckAleatoriamente(partida.criarCard(TOCHA_VORAZ,
                    System.nanoTime()));
        }
    }
    ...
    private static void loe_026() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            mortos = Filtro.raca(mortos, MURLOC);
            if (mortos.size() < 7) {
            ...
            for (Card evocar : mortos) {
                hero.evocar(partida.criarCard(evocar.getId(),
                        System.nanoTime()));
            }
        }
    }
    ...
    private static void loe_019t() {
        feitico.getDono().addCardDeckAleatoriamente(
                partida.criarCard(MACACO_DOURADO, System.nanoTime()));
    ...
    private static void loea16_5() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            while (hero.temEspacoNaMesa()) {
                hero.evocar(partida.criarCard(MUMIA_ZUMBI3_3, System.nanoTime()));
            }
        }
    }
    ...
    private static void loe_111() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            EmArea.dano(partida, partida.getMesa(),
                    Utils.getDanoFeitico(3, hero), 0);
            oponente.addCardDeckAleatoriamente(
                    partida.criarCard("LOE_111", System.nanoTime()));
        }
    }
    ...
    private static void new1_031() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(Random.companheiroAnimal(),
                    System.nanoTime()));
        }
    }
    ...
    private static void tt_010(Card segredo) throws JogouCardException {
        if (alvoCard != null) {
            GameCliente.exibirSegredoRevelado(segredo.getId(),
                    segredo.getDono().getHeroi());
            Card alvo = partida.criarCard(DOBRADORA_DE_FEITICOS,
                    System.nanoTime());
            segredo.getDono().evocar(alvo);
            MENSAGEM += " o alvo do feitiço " + feitico.getName()
                    + " foi alterado para " + alvo.getName()
                    + " pelo " + segredo.getName();
    ...
    private static void pro_001a() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            ...
            for (int i = 0; i < quantidade; i++) {
                hero.evocar(partida.criarCard(MURLOC1_1, System.nanoTime()));
            }
        }
    }
    ...
    private static void pro_001c() throws JogouCardException {
        if (!anularAlterarFeitico()) {
            hero.evocar(partida.criarCard(Random.guerreiroDaHorda(),
                    System.nanoTime()));
        }
    }
    ...
    public static boolean anularSegredo(Partida partida, Card segredoAnulado) {
        UtilizarFeitico.partida = partida;
        List<Card> list = new ArrayList<>();
        segredoAnulado.getOponente().getSegredo().stream().forEach((segredo) -> {
            list.add(segredo);
        });
        for (int i = 0; i < list.size(); i++) {
            switch (list.get(i).getId()) {
