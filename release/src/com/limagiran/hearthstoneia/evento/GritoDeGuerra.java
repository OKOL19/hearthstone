...
public class GritoDeGuerra {
    ...
    private static void at_081() {
        for (Card lacaio : card.getOponente().getMesa()) {
            lacaio.setAtaque(1, true);
        }
    }
    ...
    private static void gvg_108() throws JogouCardException {
        if (escolhido == EXECUTAR_GRITO_DE_GUERRA
            ...
            if (escolhido != EscolherMesa.CANCEL) {
                ...
                partida.polimorfia(escolhido,
                        Random.getCard(LACAIO, GERAL, custo, GERAL, GERAL, GERAL).getId(), true);
            } else {
                throw new JogouCardException("Grito de guerra cancelado.");
            }
        }
    ...
    private static void loe_003() {
        card.getDono().addMao(partida.criarCard(Utils.descobrir("Descubra um feitiço",
                com.limagiran.hearthstone.util.Utils.getIds(Random.listOnlyFeitico(3))), System.nanoTime()));
    }
    ...
    private static void loe_011() {
        ...
        for (Card c : card.getDono().getDeck()) {
            list.add(c.getId());
        }
    ...
    private static void new1_030() {
        ...
        for (Card descarte : descartar) {
            card.getDono().descartar(descarte);
        }
    }
