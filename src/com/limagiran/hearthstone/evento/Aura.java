...
public class Aura {
    ...
    public static void refresh(Partida partida) {
        ...
        list.stream().filter((card) -> (card != null)).forEach((card) -> {
        ...
        list.stream().filter((card) -> (!card.isSilenciado()))
                .forEach((card) -> {
                    exeSwitch(card);
                });
        for (Card card : list) {
        ...
        list.stream().filter((c) -> (c != null)).map((card) -> {
    ...
    private static void exeSwitch(Card card) {
        switch (card.getId()) {
    ...
    private static void at_045(Card card) {
        if (card.isMesa()) {
            card.getDono().getMao().forEach(c -> c.getAura().delCusto(c.getCusto() - 1));
        }
    }
    ...
    private static void at_070(Card card) {
        if (card.isMao()) {
            int quantidade = (int) card.getDono().getMesa().stream().filter(Card::isPirata).count();
            if (quantidade > 0) {
    ...
    private static void at_075(Card card) {
        if (card.isMesa()) {
            card.getDono().getMesa().stream()
                    .filter(lacaio -> (lacaio.getId().equals(Values.RECRUTA_PUNHO_DE_PRATA)))
                    .forEach(lacaio -> lacaio.getAura().addAtaque(1));
        }
    }
    ...
    private static void cs2_222(Card card) {
        if (card.isMesa()) {
            card.getDono().getMesa().forEach(lacaio -> {
    ...
    private static void cs2_227(Card card) {
        if (card.isMesa()) {
            card.getDono().getMao().stream()
                    .filter(Card::isLacaio)
                    .forEach(c -> c.getAura().addCusto(3));
        }
    }
    ...
    private static void ex1_162(Card card) {
        if (card.isMesa()) {
            Utils.getAdjacentes(card).forEach(adjacente -> adjacente.getAura().addAtaque(1));
        }
    }
    ...
    private static void ex1_507(Card card) {
        if (card.isMesa()) {
            partida.getMesa().forEach(lacaio -> {
    ...
    private static void ex1_565(Card card) {
        if (card.isMesa()) {
            Utils.getAdjacentes(card).forEach(adjacente -> adjacente.getAura().addAtaque(2));
        }
    }
    ...
    private static void ex1_608(Card card) {
        if (card.isMesa()) {
            card.getDono().getMao().stream()
                    .filter(Card::isFeitico)
                    .forEach(mao -> mao.getAura().delCusto(1));
        }
    }
    ...
    private static void ex1_616(Card card) {
        if (card.isMesa()) {
            partida.getMao().stream()
                    .filter(Card::isLacaio)
                    .forEach(mao -> mao.getAura().addCusto(1));
        }
    }
    ...
    private static void fp1_017(Card card) {
        if (card.isMesa()) {
            partida.getMao().stream()
                    .filter(Card::isLacaio)
                    .filter(Card::isGritoDeGuerra)
                    .forEach(mao -> mao.getAura().addCusto(2));
        }
    }
    ...
    private static void gvg_122(Card card) {
        if (card.isMesa()) {
            Utils.getAdjacentes(card).forEach(adjacente -> adjacente.getAura().setImuneAlvo(true));
        }
    }
    ...
    private static void loe_038(Card card) {
        if (card.isMesa()) {
            card.getDono().getMao().forEach(mao -> {
                if (mao.getCusto() < 5) {
    ...
    private static void loe_113(Card card) {
        if (card.isMao()) {
            int quantidade = (int) card.getDono().getMesa().stream().filter(Card::isMurloc).count();
            if (quantidade > 0) {
                card.getAura().delCusto(quantidade);
            }
        }
    ...
    private static void new1_033(Card card) {
        if (card.isMesa()) {
            card.getDono().getMesa().stream()
                    .filter(aliado -> !aliado.equals(card))
                    .forEach(aliado -> aliado.getAura().addAtaque(1));
        }
    }
    ...
    private static void new1_027(Card card) {
        if (card.isMesa()) {
            card.getDono().getMesa().forEach(mesa -> {
    ...
    private static void ex1_062(Card c) {
        if (c.isMesa()) {
            int quantidade = (int) c.getDono().getMesa().stream().filter(lacaio -> (!lacaio.equals(c) && lacaio.isMurloc())).count();
            if (quantidade > 0) {
