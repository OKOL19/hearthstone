...
import com.limagiran.hearthstone.heroi.view.View;
import com.limagiran.hearthstone.util.*;
import com.limagiran.hearthstone.card.control.Aura;
...
public final class Heroi extends View implements Serializable {
    ...
    private int shield = GamePlay.INSTANCE.getShieldInicial();
    private int mana = 0;
    ...
    public static Heroi getDefault(long heroi) {
        if (instanceDefault == null) {
            instanceDefault = new Heroi("", heroi, Values.MAGO, new ArrayList<>(), new CardBack(1));
        }
        return instanceDefault;
    }

    public Heroi(String nome, long heroi, String type, List<Card> deck, CardBack cardback) {
        this.nome = nome;
        this.heroi = heroi;
        this.type = type;
        poderHeroico = PoderHeroico.getDefault(getInstance());
        vidaTotal = GamePlay.INSTANCE.getVidaTotalHeroi();
        health = vidaTotal;
        this.deck = deck;
    ...
    public void jogarCarta(long id_long, int index) {
        ...
        if (card != null) {
            ...
            try {
                ...
                delMao(card);
                if (!card.isSegredo()) {
                    GameCliente.send(ANIMACAO_CARD_JOGADO, card.getId());
    ...
    private void evocarComGritoDeGuerra(int posicao, Card lacaio) throws JogouCardException {
        if (getMesa().size() < 7) {
            ...
            GameCliente.send(ANIMACAO_CARD_EVOCADO, lacaio.id_long);
            lacaio.getAnimacao().evocado();
            try {
        ...
        if (!deck.isEmpty()) {
            if (index >= 0 && index < deck.size()) {
                ...
                if (mode != Card.COMPRA_INICIO_PARTIDA) {
                    if (getMao().size() >= GamePlay.INSTANCE.getCartasNaMao()) {
                ...
                if (getMao().size() >= GamePlay.INSTANCE.getCartasNaMao()) {
                    getPartida().addHistorico(getNome()
                            + " teve o card queimado pois sua mão está cheia!\n"
                            + Utils.getDescricao(getNome(), comprado));
                } else {
                    switch (mode) {
                        case Card.COMPRA_PODER_HEROICO:
                            addMao(comprado);
                            //Walfrido Crepistala (Os cards que você compra com o seu Poder Heroico custam 0)
                            if (temNaMesa("AT_027")) {
    ...
    public void setArma(Card arma) {
        if (getArma() != null) {
            Audios.playEfeitos(Audios.PARTIDA_ARMA_DESTRUIR);
            Utils.sleep(2500);
        ...
        this.arma = arma;
        getPanelArma().removeAll();
        if (getArma() != null) {
            Audios.playEfeitos(Audios.PARTIDA_ARMA_EQUIPAR);
            Utils.sleep(700);
            ...
            arma.setArmaAtivada(getPanelAtaque().isInvestida());
            getPanelArma().setArma(getArma());
            setAtaqueVisible((getHeroi() == HEROI && getPartida().isVezHeroi())
                    || getHeroi() == OPONENTE && !getPartida().isVezHeroi());
        } else {
            setAtaqueVisible(false);
        }
    }
    ...
    private void setWeaponComGritoDeGuerra(Card arma) throws JogouCardException {
        ...
        setArma(arma);
        GameCliente.send(getHeroi() != HEROI
                ? HEROI_SET_ARMA
                : OPONENTE_SET_ARMA, this.arma == null ? "null" : this.arma.id_long);
        if (getArma() != null) {
    ...
    public void delHealth(int damage) {
        ...
        if (getHealth() <= 0) {
            Audios.playEfeitos(Audios.PARTIDA_HEROI_MORREU_1);
            Audios.playEfeitos(Audios.PARTIDA_HEROI_MORREU_2);
    ...
    public void setShield(int shield, boolean flag) {
        if (this.shield <= 0 && shield > 0) {
            Audios.playEfeitos(Audios.PARTIDA_HEROI_ESCUDO_GANHAR);
        } else if (this.shield > 0 && shield <= 0) {
            Audios.playEfeitos(Audios.PARTIDA_HEROI_ESCUDO_PERDER);
        }
        this.shield = shield;
        if (flag) {
            GameCliente.send(getHeroi() != HEROI
        ...
        getPanelEscudo().setVisible(this.shield > 0);
    }
    ...
    public void setMana(int mana) {
        if (mana <= 10) {
            ...
            getLabelMana().setText((getManaDisponivel() < 0 ? 0 : getManaDisponivel()) + " / " + getMana());
            getPanelMana().atualizar();
        }
    }
    ...
    public void setManaUtilizada(int manaUtilizada) {
        ...
        getLabelMana().setText(getManaDisponivel() + " / " + getMana());
        getPanelMana().atualizar();
    }
    ...
    public void setAtaquesRealizados(int ataques_realizados) {
        ...
        if (getArma() != null) {
            getArma().viewCardMesa.setArmaAtivada(isInvestidaView());
        }
    ...
    public void refreshDanoMagico() {
        new Thread(() -> {
            while (getPartida().getPartidaView().gameCliente.onLine) {
    ...
    public void finalizar() {
        ...
        clear(segredo);
        clear(morto);
        clear(cardsJogadosNaRodada);
        clear(lacaiosMortosNaRodada);
        Heroi h = getInstance();
        h = null;
    }

    private void clear(List<Card> cards) {
        for (Card card : cards) {
            card.finalizar();
            card = null;
        }
     ...
     * @throws com.limagiran.hearthstone.heroi.control.JogouCardException "Você não
     * pode ter 2 segredos iguais em jogo!"
     */
    private void utilizarSegredo(Card segredo) throws JogouCardException {
        if (temSegredoAtivado(segredo.getId())) {
            GameCliente.showPopUp("Você não pode ter 2 segredos iguais em jogo!");
            ...
            if (!UtilizarFeitico.anularSegredo(getPartida(), segredo)) {
                GameCliente.send(ANIMACAO_CARD_SEGREDO_ATIVADO, "null");
                Audios.playEfeitos(Audios.PARTIDA_SEGREDO_ATIVAR);
                addSegredo(segredo);
            } else {
                GameCliente.send(ANIMACAO_CARD_JOGADO, segredo.getId());
            }
